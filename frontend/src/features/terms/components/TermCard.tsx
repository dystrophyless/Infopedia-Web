...
export function TermCard({ term, relatedTerms = [], backTo = '/search', selected = false, onSelectedChange, expansion = 'intrinsic' }: TermCardProps) {
  ...
  return (
    <div className={`group/inspect relative hidden min-w-0 items-start md:flex ${selected ? (expansion === 'fill-parent' ? 'w-full max-[1100px]:flex-wrap' : 'w-[951px]') : 'w-[684px] hover:w-[716px] focus-within:w-[716px]'}`} data-term-card-state={selected ? 'clicked' : 'default'}>
      <div className="relative flex min-h-[208px] h-auto w-[684px] shrink-0 flex-col gap-8 rounded-[16px] bg-white p-6 text-[#161519]" data-term-card-main>
