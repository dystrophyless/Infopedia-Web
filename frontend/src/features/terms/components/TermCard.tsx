...
export function TermCard({ term, backTo = '/search', selected = false, onSelectedChange, expansion = 'intrinsic' }: TermCardProps) {
  ...
  const inspectTitleIsLong = displayName.length > 48;
  const detailLink = <Link to={`/terms/${term.public_id}`} state={{ backTo, term, selectedDefinitionPublicId: definition?.public_id }} className="flex h-10 w-[216px] shrink-0 items-center justify-center rounded-[8px] bg-[#6a37c3] px-8 text-[16px] font-medium leading-4 text-[#efeaf8] outline-none focus-visible:ring-2 focus-visible:ring-[#6a37c3] focus-visible:ring-offset-2">{t('terms.inspectDetailsCta')}</Link>;
      ...
      <div className="relative flex min-h-[208px] h-auto w-full max-w-[684px] shrink-0 flex-col gap-8 rounded-[16px] bg-white p-6 text-[#161519]" data-term-card-main>
        <div className="flex min-h-0 flex-1 flex-col gap-4" data-term-card-body>
          <div className="flex min-h-6 items-start justify-between gap-12">
            <button type="button" data-term-card-title className="min-w-0 flex-1 rounded-[8px] text-left outline-none focus-visible:ring-2 focus-visible:ring-[#6a37c3] focus-visible:ring-offset-2" aria-label={displayName} aria-pressed={selected} onClick={() => onSelectedChange?.(!selected)}>
              <span className={`block truncate font-medium ${inspectTitleIsLong ? 'text-[20px] leading-5' : 'text-[22px] leading-6'}`}>{displayName}</span>
            </button>
