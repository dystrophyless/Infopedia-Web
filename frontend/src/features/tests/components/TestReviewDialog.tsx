...
export function TestReviewDialog({ open, question, questionNumber, totalQuestions, feedback, onDismiss }: { open: boolean; question: TestQuestion | null; questionNumber: number; totalQuestions: number; feedback: TestAnswerFeedback | null; onDismiss: () => void }) {
  ...
  return <Dialog id="test-review-dialog" open={open} onDismiss={onDismiss} titleId="test-review-title" initialFocusRef={closeRef} overlayClassName="!bg-[rgba(22,21,25,.25)]" className="flex max-h-[calc(100vh-32px)] !w-[656px] max-w-[calc(100vw-32px)] flex-col overflow-hidden !rounded-[16px] bg-white p-0">
    <div className="flex shrink-0 items-start gap-8 px-8 pt-8"><div className="min-w-0 flex-1"><p data-figma-contrast-lock="review-meta" className="text-[16px] font-medium leading-4 text-[#c5b1e7]">{t('tests.desktopQuestionProgress', { current: questionNumber, total: totalQuestions })}</p><h2 id="test-review-title" className="mt-4 break-words text-[20px] font-medium leading-5 text-[#161519]">{question.prompt}</h2></div><button ref={closeRef} type="button" onClick={onDismiss} aria-label={t('tests.desktopCloseReview')} className="flex size-11 shrink-0 items-center justify-center rounded-[4px] text-[#6e6779] outline-none transition-colors hover:bg-[#f8f5fc] focus-visible:ring-2 focus-visible:ring-[#6a37c3]"><HugeiconsIcon icon={Flag02Icon} size={24} /></button></div>
    <div className="min-h-0 flex-1 overflow-x-hidden overflow-y-auto overscroll-contain px-8 pb-8"><div className="mt-10 flex flex-col gap-2">{question.options.map(option => <DesktopTestAnswerOption key={option.id} option={option} locked tone={getOptionTone({ optionId: option.id, selectedOptionId: feedback?.optionId ?? null, checkedOptionId: feedback?.optionId ?? null, correctOptionRef: feedback?.correctOptionRef, answerCorrect: feedback?.correct })} onSelect={() => undefined} />)}</div>
    {feedback && <section className={`mt-6 min-h-[99px] rounded-[8px] px-6 py-4 ${feedback.correct ? 'bg-[#e7f8f0] text-[#21835a]' : 'bg-[#fce5e3] text-[#9a2219]'}`}><h3 data-figma-contrast-lock={feedback.correct ? undefined : 'review-feedback-wrong-title'} className={`text-[18px] font-medium leading-[18px] ${feedback.correct ? 'text-[#29ae70]' : 'text-[#f25f54]'}`}>{feedback.correct ? t('tests.desktopCorrect') : t('tests.desktopIncorrect')}</h3><p className="mt-4 break-words text-[16px] leading-4">{feedback.explanation ?? question.explanation}</p></section>}
    </div>
  </Dialog>;
}
