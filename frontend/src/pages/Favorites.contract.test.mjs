...
const defaultSkeleton = skeleton.slice(skeleton.lastIndexOf('return ('));
...
assert.match(page, /export function FavoritesContent\(\{[\s\S]*embedded = false[\s\S]*detailBackTo = '\/favorites'/, 'Favorites should expose an embedded content presentation');
assert.match(page, /return embedded \? content : \(\s*<MobilePageFrame/, 'Standalone Favorites must retain its route frame while embedded mode suppresses it');
assert.match(page, /!embedded && <header/, 'Embedded Favorites must suppress the standalone desktop header');
...
assert.match(page, /embedded\s*\?\s*'flex h-\[416px\] min-h-\[416px\] max-h-\[416px\][^']*overflow-hidden/, 'Embedded Favorites must use the fixed 416px desktop panel');
assert.match(page, /embedded \? '[^']*min-h-0 flex-1 overflow-y-auto/, 'Embedded populated Favorites must scroll internally');
assert.match(page, /!isLoading && error && list\.length === 0[\s\S]*BetweenBlocks[\s\S]*role="alert"/, 'Embedded initial errors must share the structural sandwich');
assert.match(page, /showEmptyState && \([\s\S]*BetweenBlocks[\s\S]*min-h-0 flex-1/, 'Embedded empty outcomes must keep equal structural insets');
assert.match(page, /error && list\.length > 0[\s\S]*onClick=\{loadMore\}/, 'Embedded load-more errors must keep a reachable retry');
...
assert.match(page, /detailBackTo = '\/favorites'/, 'Standalone favorites must default term-detail links back to favorites');
assert.match(page, /className="hidden flex-col gap-4 md:flex"[\s\S]*<TermCard[\s\S]*backTo=\{detailBackTo\}/, 'Desktop favorites must retain the current TermCard branch');
assert.match(page, /className="hidden flex-col gap-4 max-md:-mx-\[2px\] max-md:flex max-md:w-\[calc\(100%\+4px\)\]"[\s\S]*<MobileSearchTermCard[\s\S]*backTo=\{detailBackTo\}/, 'Mobile favorites must restore the dedicated filled-state term card');
...
assert.match(defaultSkeleton, /data-skeleton-card[^>]*className="[^"]*min-h-\[208px\][^"]*w-\[684px\][^"]*rounded-\[16px\][^"]*bg-white[^"]*p-6/, 'Default skeleton must mirror the collapsed desktop TermCard shell');
assert.match(defaultSkeleton, /data-skeleton-header[\s\S]*data-skeleton-title[\s\S]*data-skeleton-header-actions/, 'Default skeleton must reserve the title and header controls');
assert.match(defaultSkeleton, /data-skeleton-definition/, 'Default skeleton must reserve the collapsed definition preview');
assert.match(defaultSkeleton, /data-skeleton-cta[^>]*className="[^"]*h-10[^"]*w-\[216px\]/, 'Default skeleton CTA must match the 216x40 desktop TermCard action');
assert.doesNotMatch(defaultSkeleton, /\bborder(?:-|\s)|data-skeleton-metadata|data-skeleton-actions|data-skeleton-action/, 'Default skeleton must omit the legacy border, metadata row, and bottom action controls');
...
assert.match(page, /isLoading && list\.length === 0[\s\S]*embedded \? 'hidden min-h-0 flex-1 space-y-4 overflow-y-auto md:block'[\s\S]*<SkeletonCard[\s\S]*className="hidden space-y-4 max-md:-mx-\[2px\] max-md:block max-md:w-\[calc\(100%\+4px\)\]"[\s\S]*variant="mobile-term-card"/, 'Favorites loading state must render breakpoint-exclusive default and mobile skeleton branches on the populated-card rail');
...
assert.match(search, /to="\/favorites"[\s\S]*search\.favoritesAria/, 'Search header CTA must navigate to favorites');
  assert.match(termCard, /<TermCardFavoriteButton[\s\S]*termRef=\{term\.public_id\}/, 'Desktop TermCard must use the local favorite control');
...
assert.match(story, /<MemoryRouter initialEntries=\{\['\/favorites'\]\}>[\s\S]*<FavoritesPage \/>/, 'Favorites stories must exercise the real route frame and page without nesting a second Layout main');
assert.doesNotMatch(story, /<Layout>/, 'Favorites stories must not nest the global Layout around the page-owned MobilePageFrame');
