...
assert.match(analyze, /data-analysis-steps/);
assert.match(analyze, /grid-cols-1[\s\S]*lg:grid-cols-3[\s\S]*lg:items-end[\s\S]*lg:gap-\[clamp\(32px,4vw,64px\)\]/);
...
assert.match(analyze, /transition-transform[\s\S]*hover:scale-\[1\.01\][\s\S]*motion-reduce:hover:scale-100/);
assert.doesNotMatch(analyze, /xl:absolute|xl:left-\[|xl:top-\[/);
