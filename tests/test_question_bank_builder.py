...
def _question(
    *, prompt: str = "Question", options: tuple[str, ...] = ("A", "B", "C", "D")
) -> dict:
    return {
        "question": prompt,
...
def _write_fixture_tree(tmp_path: Path) -> tuple[Path, Path]:
    ...
    payload = {
        ...
        "topics": [
            ...
            {
                "topic": "Missing",
                "questions": [_question(options=("x", "x", "y", "z"))],
            },
        ],
    }
...
class QuestionBankBuilderTests(unittest.TestCase):
    def test_builder_uses_stable_question_keys_and_occurrence_suffix(self):
        with tempfile.TemporaryDirectory() as directory:
            source_dir, structure_path = _write_fixture_tree(Path(directory))

            artifact = build_question_bank(
                source_dir, structure_path, contract=_fixture_contract()
            )

        first_key, second_key = (
            item["source_key"] for item in artifact["questions"][:2]
        )
        ...
        self.assertNotIn(
            "chapter", json.dumps(artifact["questions"], ensure_ascii=False).casefold()
        )
    ...
    def test_atomic_writer_keeps_previous_artifact_when_build_fails(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(ValueError, "Malformed"):
                write_question_bank_atomic(
                    source_dir, structure_path, output, contract=_fixture_contract()
                )

            self.assertEqual(output.read_text(encoding="utf-8"), "previous")

    def test_generated_production_artifact_has_revised_contract_counts(self):
            ...
            frozenset(
                {
                    ...
                    (
                        "Алматыкітап: 9-сынып",
                        "5.2. Артық фон мен ойын кейіпкерлері",
                        15,
                    ),
                },
            ),
    ...
    def test_generated_artifact_has_expected_distinct_and_per_chapter_eligibility_counts(
        self,
    ):
        entries = load_question_bank(ROOT / "src" / "data" / "questions.json")
        structure = json.loads(
            (ROOT / "src" / "data" / "newStructure.json").read_text(encoding="utf-8")
        )
        mapping = json.loads(
            (ROOT / "src" / "data" / "mappingStructure.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        distinct_active = sum(
            entry.active and bool(chapters)
